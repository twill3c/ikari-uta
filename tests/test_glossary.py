...
MEASURED_STEM_SHARING = {
    ...
    ("ロドス", "Ῥόδος", "Ῥόδιοι"),
    # loop_006(第 5 巻)で追加。**民族/地名の語幹共有ではなく、同名異物**。
    # 一覧の Φαιστός(クレータの町)と、第 5 巻で討たれる Φαῖστος(マイオニアの人)。
    # 原文の綴りも一字違いで、日本語では同じカタカナになる。対訳表に注記済み。
    ("パイストス", "Φαῖστος", "Φαιστός"),
    # loop_006 でリュキエーの川 Ξάνθος とパイノプスの子 Ξάνθος の二者を挙げた。
    # loop_017(第 16 巻)でアキレウスの不死の馬 Ξάνθος が加わり**三者**になった。
    # T-029b は ja をキーに直前の一件と突き合わせるので、三者は連鎖して**二組**を生む。
    # つまり loop_006 の組 ("クサントス", "Ξάνθος(人)", "Ξάνθος") は消え、下の二組に置き換わる。
    # 消えたこと自体は T-029b の「消えた」側が正しく報告した(据え置きでは通らない)。
    ("クサントス", "Ξάνθος(人)", "Ξάνθος(馬)"),
    ("クサントス", "Ξάνθος(馬)", "Ξάνθος"),
    # loop_017。第 6 巻の人 Πήδασος と、アキレウスの添え馬 Πήδασος。
    ("ペーダソス", "Πήδασος", "Πήδασος(馬)"),
